...
class DistTag extends BaseCommand {
  ...
  async add (spec, tag, opts) {
    ...
    await this.npm.registry.putJson(url, version, { ...opts, spec })
  ...
  async remove (spec, tag, opts) {
    ...
    await this.npm.registry.fetch(url, reqOpts)
  ...
  async list (spec, opts) {
    ...
    try {
      const tags = await this.npm.registry.fetchJson(
        `/-/package/${spec.escapedName}/dist-tags`,
        { ...opts, spec, preferOnline: true }
      )
      const msg =
  ...
  async fetchTags (spec, opts) {
    const data = await this.npm.registry.fetchJson(
      `/-/package/${spec.escapedName}/dist-tags`,
