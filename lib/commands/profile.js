...
class Profile extends BaseCommand {
  ...
  async set (args) {
    ...
    const result = await this.npm.registry.withOtp(c => set(newUser, c), conf)
  ...
  async enable2fa (args) {
    ...
    const challenge = await this.npm.registry.withOtp(o => set(info, o), conf)
  ...
  async disable2fa () {
    ...
    await this.npm.registry.withOtp(o => set({ tfa: { password: password, mode: 'disable' } }, o), opts)
