const { adduserWeb, loginWeb, loginCouch, adduserCouch } = require('npm-profile')
...
const adduser = async (npm, { creds, ...opts }) => {
  ...
  if (!res) {
    ...
    // be prompted for it.
    res = await npm.registry.withOtp(
      (reqOpts) => adduserCouch(username, email, password, reqOpts),
      opts
    )
  }
...
const login = async (npm, { creds, ...opts }) => {
  ...
  if (!res) {
    ...
    const password = await read.password('Password:', creds.password)
    res = await npm.registry.withOtp(
      (reqOpts) => loginCouch(username, password, reqOpts),
      opts
    )
  }
