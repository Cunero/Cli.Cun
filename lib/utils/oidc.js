...
async function oidc ({ packageName, registry, opts, config }) {
  ...
  try {
    if (!(
      ...
      log.silly('oidc', 'Skipped because not running in a supported CI environment')
      return undefined
    }
    ...
    if (idToken) {
      ...
      if (ciInfo.GITHUB_ACTIONS) {
        ...
        if (
          ...
          log.silly('oidc', 'Skipped with GITHUB_ACTIONS detected because incorrect permissions for id-token within workflow')
          return undefined
        }
      }
    }
    ...
    try {
      ...
      if (error?.body?.message) {
        log.verbose('oidc', `Failed with registry body response error message "${error.body.message}"`)
      }
      return undefined
    }

    if (!response?.token) {
      log.verbose('oidc', 'Failed with token exchange missing token in response body')
      return undefined
    }
    /*
    ...
    log.verbose('oidc', 'Failed checking config', error)
  }
  return undefined
}
